...
public class QuestPdfDocumentRenderer : IDocumentRenderer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // The invoice template has a dedicated layout. Other report templates must
        // not silently render as an invoice; use a deterministic tabular fallback.
        if (order is null || !templateCode.Equals("Invoice_A4", StringComparison.OrdinalIgnoreCase))
        {
            return Task.FromResult(RenderGenericPdf(templateCode, compiledHtml, dataModel ?? new { }));
        }
    ...
    private static byte[] RenderGenericPdf(string templateCode, string compiledHtml, object dataModel)
    {
        var root = JsonSerializer.SerializeToElement(dataModel);
        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Text(templateCode).Bold().FontSize(15);
                page.Content().Column(column =>
                {
                    column.Item().PaddingBottom(8).Text($"Generated at {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC");
                    if (columns.Count == 0)
                    {
                        column.Item().Text("No tabular data was returned.");
                        return;
                    }
                    ...
                    column.Item().Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            {
                                header.Cell().Background(Colors.Grey.Lighten3).Padding(4).Text(columnName).Bold();
                            }
                        });
                        ...
                        foreach (var row in rows.Take(1000))
                        {
                            foreach (var columnName in columns)
                            {
                                var value = row.ValueKind == JsonValueKind.Object && row.TryGetProperty(columnName, out var property)
                                    ? property.ToString()
                                    : string.Empty;
